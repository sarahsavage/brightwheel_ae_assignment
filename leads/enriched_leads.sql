...
, case when sf_leads.surrogate_key is not null
     ...
     when (s1_leads.surrogate_key is not null)::int + (s2_leads.surrogate_key is not null)::int
        + (s3_leads.surrogate_key is not null) > 1  then 'Multiple'
        else 'Unknown'                                                                                  as lead_origin
     ...
     , s2.primary_caregiver                                                                             as s2_primary_caregiver
     , s2.role                                                                                          as s2_caregiver_role
     ...
     , coalesce(sf_leads.phone, addresses.phone, permits.phone, schedules.phone)                        as phone
     , sf_leads.mobile_phone                                                                            as sf_mobile_phone
     , sf_leads.website
     , coalesce(sf_leads.center_capacity, s2.center_capacity,
                s3.center_capacity)                                                                     as center_capacity
     ...
     , sf_lead.created_at                                                                               as sf_lead_created_at
     , sf_lead.last_modified_at                                                                         as sf_lead_modified_at
     , sf_lead.last_activity_date                                                                       as sf_lead.last_activity_date
